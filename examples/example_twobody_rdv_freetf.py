...
def test_scp_scipy_logmass(get_plot=False):
    ...
    x0 = np.array([1.0, 0.0, 0.0002, 0.003, 1.0, 0.0, np.log(1.0)])
    oe0 = pk.ic2par(x0[0:3], x0[3:6], mu)
    period_0 = 2*np.pi*np.sqrt(oe0[0]**3/mu)
    sol_lpo0 = integrator_timedomain.solve([0, period_0], x0[0:6], get_ODESolution=True)

    xf = np.array([1.5, -0.2, 0.04, 0.0, np.sqrt(1/1.5), -0.04])
    oef = pk.ic2par(xf[0:3], xf[3:6], mu)
    period_f = 2*np.pi*np.sqrt(oef[0]**3/mu)
    sol_lpo1 = integrator_timedomain.solve([0, period_f], xf[0:6], get_ODESolution=True)
    ...
    elements = np.concatenate((
        np.linspace(oe0[0], oef[0], N).reshape(-1,1),
        np.linspace(oe0[1], oef[1], N).reshape(-1,1),
        np.linspace(oe0[2], oef[2], N).reshape(-1,1),
        np.linspace(oe0[3], oef[3], N).reshape(-1,1),
        np.linspace(oe0[4], oef[4], N).reshape(-1,1),
        np.linspace(oe0[5], oef[5], N).reshape(-1,1),
    ), axis=1)
    elements[:,5] = np.linspace(oe0[5], oef[5]+2*np.pi, N)
    xbar = np.zeros((N,8))
    xbar[:,0:6] = np.array(
    ...
    xbar[-1,0:6]  = xf[0:6]          # overwrite final state
