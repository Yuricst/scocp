...
def get_heyoka_integrator_twobody_logmass(
    mu,
    cex,
    tol = 1e-12,
    high_accuracy = True,
...
def get_heyoka_integrator_twobody_mass(
    mu,
    c1,
    c2,
    tol = 1e-12,
    ...
    Paramers are:
    hy.par = [mu, c1, c2, ux, uy, uz, s, Gamma]
    where the last 5 entries are control inputs.
    """
    tstart = time.time()
    ...
    # This will contain the r.h.s. of the equations
    a_max = hy.par[1]/x[6]   # maximum acceleration = c1/mass
    r = (x[0]**2 + x[1]**2 + x[2]**2)**(1/2.)
    f = []
    f.append(hy.par[6] * x[3])                                               # dx/dtau
    f.append(hy.par[6] * x[4])                                               # dy/dtau
    f.append(hy.par[6] * x[5])                                               # dz/dtau
    f.append(hy.par[6] * (-hy.par[0] * x[0] / r**3 + a_max * hy.par[3]))     # dvx/dtau
    f.append(hy.par[6] * (-hy.par[0] * x[1] / r**3 + a_max * hy.par[4]))     # dvy/dtau
    f.append(hy.par[6] * (-hy.par[0] * x[2] / r**3 + a_max * hy.par[5]))     # dvz/dtau
    f.append(hy.par[6] * (-hy.par[2] * hy.par[7]))                           # dmass/dtau
    f.append(hy.par[6])                                                      # dt/dtau
    ...
    for i in range(nx):
        for j in range(nu):
            dfdu.append(hy.diff(f[i],hy.par[j+3]))
    dfdu = np.array(dfdu).reshape((nx,nu))
    ...
    ta_dyn_aug.pars[0] = mu
    ta_dyn_aug.pars[1] = c1
    ta_dyn_aug.pars[2] = c2
    ta_dyn_aug.pars[3] = 0.0
    ta_dyn_aug.pars[4] = 0.0
    ta_dyn_aug.pars[5] = 0.0
    ta_dyn_aug.pars[6] = 1.0
    ta_dyn_aug.pars[7] = 0.0
    if verbose:
        print(f"Done! Elapsed time: {time.time() - tstart:1.4f} seconds")
    return ta_dyn, ta_dyn_aug